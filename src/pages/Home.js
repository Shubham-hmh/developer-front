            ...
            productState && productState?.map((item, index) => {
              if (item.tags === "special") {
                return (
                  ...
                  <SpecialProduct key={index} id ={item?._id} sold={item?.sold} quantity={item?.quantity} images={item?.images[0].url}brand={item?.brand} title={item?.title} price={item?.price} totalrating={item?.totalrating} />
                )
              }
